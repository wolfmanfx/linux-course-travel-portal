window.COURSE_CONTENT = {
  "generatedAt": "2026-07-18T10:45:24.788Z",
  "narration": {
    "language": "en",
    "complete": false,
    "availableSlides": 38,
  ...
  "videos": [
    {
      ...
      "ready": true,
      "language": "en",
      "master": "media/lab01-system-orientation.mp4",
      "audio": "media/lab01-system-orientation.en.mp3",
      "captions": "media/lab01-system-orientation.en.vtt",
      "captionsSrt": "media/lab01-system-orientation.en.srt",
      "video": "media/lab01-system-orientation.en.mp4",
      "transcript": "Start with identity and location. whoami, hostname, pwd, and HOME prove which account, system, and working directory this terminal controls before we make any change.\n\nThe os-release file describes Ubuntu user space. uname reports the running kernel and machine architecture, while ps confirms that systemd is process one. These are related facts, not one version number.\n\nWe build system-profile.txt from the observed commands instead of typing guessed values. The second evidence file maps configuration, persistent logs, and runtime state to their standard filesystem locations.\n\nThe final test proves that all three directories exist, then cat displays both evidence files. The checker validates every required key and confirms that the profile describes the current seat."
    },
    {
      "id": "lab02",
      "number": 2,
      ...
      "ready": true,
      "language": "en",
      "master": "media/lab02-filesystem-and-links.mp4",
      "audio": "media/lab02-filesystem-and-links.en.mp3",
      "captions": "media/lab02-filesystem-and-links.en.vtt",
      "captionsSrt": "media/lab02-filesystem-and-links.en.srt",
      "video": "media/lab02-filesystem-and-links.en.mp4",
      "transcript": "We inspect the inbox first, then create notes, archive, and links beneath one explicit project path. The variable keeps repeated destinations readable without changing their meaning.\n\ncp preserves the welcome file at its source. mv relocates the draft, so the old draft path disappears. Those different source-side effects are part of the required final state.\n\nln without -s creates another name for the same inode. ls -li displays matching inode numbers, which proves this is a hard link rather than a second copy.\n\nThe symbolic link stores the relative target ../notes/welcome.txt. readlink proves the stored path, cat proves that it resolves, and find gives one final view of every object and link target."
    },
    {
      "id": "lab03",
      "number": 3,
      ...
      "ready": true,
      "language": "en",
      "master": "media/lab03-editor-and-text-evidence.mp4",
      "audio": "media/lab03-editor-and-text-evidence.en.mp3",
      "captions": "media/lab03-editor-and-text-evidence.en.vtt",
      "captionsSrt": "media/lab03-editor-and-text-evidence.en.srt",
      "video": "media/lab03-editor-and-text-evidence.en.mp4",
      "transcript": "First inspect the numbered configuration, then edit it in Nano. We replace all three lines, write the file with Control-O, confirm the filename, and exit with Control-X.\n\ncat proves the exact saved settings. Sorting through uniq -d checks for accidental duplicate lines before we use the configuration as evidence.\n\nThe access-log pipeline extracts field four, sorts equal status codes together, counts adjacent values, and finally rewrites each row as status followed by count. The saved report must show three 404 responses.\n\nA structured grep selects only records beginning with level=ERROR. cat displays both reports and wc confirms exactly two error events, giving independent content and count checks before automation runs."
    },
    {
      "id": "lab04",
      "number": 4,
      ...
      "ready": true,
      "language": "en",
      "master": "media/lab04-package-lifecycle.mp4",
      "audio": "media/lab04-package-lifecycle.en.mp3",
      "captions": "media/lab04-package-lifecycle.en.vtt",
      "captionsSrt": "media/lab04-package-lifecycle.en.srt",
      "video": "media/lab04-package-lifecycle.en.mp4",
      "transcript": "The initial command proves tree is missing, while apt-cache shows the repository candidate. apt update refreshes package metadata; it does not install the application.\n\napt install retrieves and registers tree through Ubuntu's supported package manager. The noninteractive yes option is used only so the recorded demonstration can proceed without a confirmation prompt.\n\ndpkg-query proves installed state, command -v locates the executable, and dpkg -S maps that file back to its owning package. These independent views should agree.\n\nWe save those four facts in package-report.txt and run tree against the prepared directory. The final output proves both package metadata and actual command behavior."
    },
    {
      "id": "lab05",
      "number": 5,
      ...
      "ready": true,
      "language": "en",
      "master": "media/lab05-permission-incident.mp4",
      "audio": "media/lab05-permission-incident.en.mp3",
      "captions": "media/lab05-permission-incident.en.vtt",
      "captionsSrt": "media/lab05-permission-incident.en.srt",
      "video": "media/lab05-permission-incident.en.mp4",
      "transcript": "Begin with accounts, group membership, and every component of the target path. Alice and Bob belong to webteam; outsider does not. The existing owner and modes explain why collaboration currently fails.\n\nchown assigns the share to webteam. Mode 2770 grants owner and group access, blocks others, and sets setgid so new files inherit the directory group. The existing checklist receives mode 660.\n\nReal operations as Alice and Bob prove the positive cases. A separate write test as outsider must fail; otherwise the design is not least privilege.\n\nThe final stat output proves numeric modes, group ownership, and inherited webteam membership on Alice's new file. The checklist content proves Bob performed a genuine group write."
    },
    {
      "id": "lab06",
      "number": 6,
      ...
      "ready": true,
      "language": "en",
      "master": "media/lab06-ssh-trust-setup.mp4",
      "audio": "media/lab06-ssh-trust-setup.en.mp3",
      "captions": "media/lab06-ssh-trust-setup.en.vtt",
      "captionsSrt": "media/lab06-ssh-trust-setup.en.srt",
      "video": "media/lab06-ssh-trust-setup.en.mp4",
      "transcript": "The private SSH directory uses mode 700. ssh-keygen creates a dedicated Ed25519 key, and mode 600 protects the private half while the public half remains shareable.\n\nssh-keyscan records the server key for port 2222.\n\nssh-keygen -F proves the bracketed host-and-port entry exists before authentication begins.\n\nssh-copy-id uses the temporary password once to install only the public key. The client profile then pins host, port, user, identity file, IdentitiesOnly, and strict host-key checking.\n\nssh -G displays the effective configuration without connecting. The final BatchMode login cannot ask for a password, so the remote id and hostname prove verified key-only access through the named profile."
    },
    {
      "id": "lab07",
      "number": 7,
      ...
      "ready": true,
      "language": "en",
      "master": "media/lab07-journal-investigation.mp4",
      "audio": "media/lab07-journal-investigation.en.mp3",
      "captions": "media/lab07-journal-investigation.en.vtt",
      "captionsSrt": "media/lab07-journal-investigation.en.srt",
      "video": "media/lab07-journal-investigation.en.mp4",
      "transcript": "systemctl status gives a compact human view of the failed unit. systemctl show then provides machine-readable ActiveState, SubState, Result, and ExecMainStatus without relying on screen formatting.\n\njournalctl scopes history to this unit and the current boot. The ISO format makes timestamps explicit while preserving the event sequence around the failure.\n\nWe save the scoped journal before filtering it. The causal record identifies incident DISK_THRESHOLD and exit code 42; the later failed state is a consequence, not the root event.\n\nlab-answer.txt records five exact facts from independent status and journal evidence. The final grep proves the preserved evidence still contains the incident submitted to the checker."
    },
    {
      "id": "lab08",
      "number": 8,
      ...
      "ready": true,
      "language": "en",
      "master": "media/lab08-broken-service-capstone.mp4",
      "audio": "media/lab08-broken-service-capstone.en.mp3",
      "captions": "media/lab08-broken-service-capstone.en.vtt",
      "captionsSrt": "media/lab08-broken-service-capstone.en.srt",
      "video": "media/lab08-broken-service-capstone.en.mp4",
      "transcript": "The capstone starts with evidence. Status and the journal show status 200/CHDIR, which points to a working-directory failure before the Python process can start.\n\nsystemctl cat and show reveal the effective User, WorkingDirectory, and ExecStart.\n\nThe smallest repair is to create the missing web root with the service account's ownership and the intended mode.\n\nWe add the exact health page and assign it to courseapp. systemctl enable --now changes both current state and next-boot state without modifying the already-correct unit file.\n\nVerification uses four independent views: active state, enabled state, the listening socket, and the HTTP response. The final stat output confirms that both directory and content ownership match the service identity."
    }
  ]
};
